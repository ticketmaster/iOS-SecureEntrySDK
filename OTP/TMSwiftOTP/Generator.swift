...
func GenerateOTP(secret: Data, algorithm: OTPAlgorithm = .sha1, counter: UInt64, digits: Int = 6) -> String {
	...
	let key = secret.withUnsafeBytes {
		[UInt8](UnsafeBufferPointer(start: $0, count: secret.count))
	}
	
	//Generate HMAC message data from counter as big endian
	let counterMessage = counter.bigEndian.toData()
	let counterMessageBytes = counterMessage.withUnsafeBytes {
		[UInt8](UnsafeBufferPointer(start: $0, count: counterMessage.count))
	}
	
	//HMAC hash counter data with secret key (counterBytes padded to 20 bytes)
	let hmac : [UInt8]!
	do {
		hmac = try HMAC(key: key, variant: algorithm.toHMACVariant()).authenticate( ( counterMessageBytes + ((counterMessageBytes.count < 20) ? Array<UInt8>(repeating: 0, count: 20 - counterMessageBytes.count) : []) ) )
	} catch {
		print("Uh oh")
		return ""
	}
